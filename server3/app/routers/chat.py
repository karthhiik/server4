    ...
    async for conv in cursor:
        for participant_id in conv.get("participants", []):
            if participant_id != user_id:
                # Notify the other user that I updated my avatar
...
async def get_conversations(user_id: str = Depends(get_current_user)):
    ...
    for conv in conversations:
        ...
        username = "unknown"
        avatar = f"https://api.dicebear.com/7.x/avataaars/svg?seed={other_user_id or 'unknown'}"
        ...
        if other_user_data:
            ...
            if other_user_data.get("photo"):
                avatar = other_user_data.get("photo")
